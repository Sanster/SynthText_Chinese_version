...
def get_text_placement_mask(xyz, mask, plane, pad=2, viz=False):
    ...
    mask_val_x = int(np.ceil(COL)) + pad
    mask_val_y = int(np.ceil(ROW)) + pad
    place_mask = 255 * np.ones((mask_val_x, mask_val_y), 'uint8')
        ...
        for i in range(ninstance):
            ...
            for idx in reg_range:
                ...
                if txt_render_res is not None:
                    ...
                    ibb.append(bb)
                    print(colorize(
                        Color.GREEN, 'text in synthgen.py/render_text append into itext ' + text))
