...
if __name__ == '__main__':
    ...
    mu = np.mean(np.reshape(
        im_src, [im_src.shape[0] * im_src.shape[1], 3]), axis=0)
    # print mu
    sz = (700, 700)
    ...
    im_actual_L = cv2.cvtColor(im_src.astype(
        'uint8'), cv2.COLOR_BGR2Lab)[:, :, 0]
    im_alpha_L = cv2.cvtColor(im_alpha.astype(
        'uint8'), cv2.COLOR_BGR2Lab)[:, :, 0]
    im_poisson_L = cv2.cvtColor(im_res.astype(
        'uint8'), cv2.COLOR_BGR2Lab)[:, :, 0]
