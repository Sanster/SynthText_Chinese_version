...
class Colorize(object):
    ...
    def drop_shadow(self, alpha, theta, shift, size, op=0.80):
        ...
        shadow = op * \
            sii.shift(shadow, shift=[dx, dy], mode='constant', cval=0)
    ...
    def color_border(self, col_text, col_bg):
        ...
        if choice == 0:
            ...
            col_text = np.squeeze(cv.cvtColor(
                col_text[None, None, :], cv.cv.CV_HSV2RGB))
        elif choice == 1:
            ...
            col_bg = np.squeeze(cv.cvtColor(
                col_bg[None, None, :], cv.cv.CV_HSV2RGB))
            col_text = np.squeeze(cv.cvtColor(
                col_text[None, None, :], cv.cv.CV_HSV2RGB))
        ...
        col_text = np.squeeze(cv.cvtColor(
            col_text[None, None, :], cv.cv.CV_RGB2HSV))
