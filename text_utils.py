...
class BaselineState(object):
    def curve(this, a): return lambda x: a * x * x

    def differential(this, a): return lambda x: 2 * a * x
    a = [0.50, 0.05]
...
class RenderFont(object):
    ...
    def render_curved(self, font, word_text):
        ...
        rots = [-int(math.degrees(math.atan(BS['diff']
        ...
        for i in range(wl):
            ...
            if i > mid_idx:
                newrect.topleft = (
                    last_rect.topright[0] + 2, newrect.topleft[1])
            else:
                newrect.topright = (
                    last_rect.topleft[0] - 2, newrect.topleft[1])
    ...
    def visualize_bb(self, text_arr, bbs):
        ta = text_arr.copy()
        for r in bbs:
            cv.rectangle(ta, (r[0], r[1]), (r[0] + r[2],
